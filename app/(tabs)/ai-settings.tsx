...
export default function AISettingsScreen() {
  ...
  const saveApiKey = async () => {
    ...
    if (!apiKey.startsWith('sk-')) {
      Alert.alert('Error', 'Please enter a valid OpenAI API key (starts with sk-)');
      return;
    }
  ...
  const toggleConsent = async () => {
    try {
      ...
      if (newConsent) {
        Alert.alert('Privacy Consent', 'Thank you for enabling data processing. Your health data will be processed locally and only aggregated insights will be shared with AI services.');
      } else {
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor }]}>
      <View style={[styles.header, { paddingTop: insets.top + 20, backgroundColor: cardBackground, borderBottomColor: borderColor }]}>
        <ThemedText type="title" style={[styles.headerTitle, { color: textColor }]}>AI Settings</ThemedText>
        <ThemedText style={[styles.headerSubtitle, { color: subtleTextColor }]}>Configure AI features and privacy</ThemedText>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.section, { backgroundColor: cardBackground, borderColor }]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="subtitle" style={[styles.sectionTitle, { color: textColor }]}>OpenAI API Key</ThemedText>
          </View>
          
          <ThemedText style={[styles.description, { color: subtleTextColor }]}>
            Enter your OpenAI API key to enable AI-powered health insights. Your key is stored securely on your device.
          </ThemedText>
          ...
          <View style={styles.buttonRow}>
            ...
            >
              <ThemedText style={styles.buttonText}>
                {isLoading ? 'Saving...' : isConfigured ? 'Update Key' : 'Save Key'}
              </ThemedText>
            </TouchableOpacity>
            ...
            {isConfigured && (
              ...
              >
                <ThemedText style={[styles.secondaryButtonText, { color: subtleTextColor }]}>Remove</ThemedText>
              </TouchableOpacity>
            )}
          </View>

          {isConfigured && (
            <View style={[styles.statusCard, { backgroundColor: inputBackground }]}>
              <IconSymbol name="checkmark.circle" size={16} color="#34C759" />
              <ThemedText style={[styles.statusText, { color: textColor }]}>AI features enabled</ThemedText>
            </View>
          )}
        </View>
        ...
        <View style={[styles.section, { backgroundColor: cardBackground, borderColor }]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="subtitle" style={[styles.sectionTitle, { color: textColor }]}>Privacy Settings</ThemedText>
          </View>

          <TouchableOpacity style={styles.settingRow} onPress={toggleConsent}>
            <View style={styles.settingInfo}>
              <ThemedText style={[styles.settingTitle, { color: textColor }]}>Data Processing Consent</ThemedText>
              <ThemedText style={[styles.settingDescription, { color: subtleTextColor }]}>
                Allow processing of health data for AI insights
              </ThemedText>
            </View>
            <View style={[styles.toggle, { backgroundColor: hasConsent ? tintColor : borderColor }]}>
              <View style={[styles.toggleThumb, { transform: [{ translateX: hasConsent ? 20 : 2 }] }]} />
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={[styles.section, { backgroundColor: cardBackground, borderColor }]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="subtitle" style={[styles.sectionTitle, { color: textColor }]}>Privacy Information</ThemedText>
          </View>

          <ThemedText style={[styles.privacyText, { color: subtleTextColor }]}>
            • Raw health data is stored encrypted on your device{'\n'}
            • Only aggregated summaries are sent to AI services{'\n'}
        ...
        <View style={[styles.section, { backgroundColor: cardBackground, borderColor }]}>
          <View style={styles.sectionHeader}>
            ...
            <ThemedText type="subtitle" style={[styles.sectionTitle, { color: textColor }]}>Data Management</ThemedText>
          </View>
