...
export class HealthAggregationService {
  ...
  async createHealthSummary(currentMetrics: HealthMetrics): Promise<HealthSummary> {
    ...
    const insights = this.generateInsights(currentMetrics, vitalityScore, weeklyTrends);
    const recommendations = this.generateRecommendations(currentMetrics, weeklyTrends);
  ...
  private generateInsights(
    ...
    vitalityScore: number,
    trends: { heartHealthTrend: string; sleepTrend: string; activityTrend: string }
  ): string[] {
    const insights: string[] = [];

    // Vitality Score Insights
    if (vitalityScore >= 90) {
      insights.push("Excellent work! You're in top form across all health metrics.");
    } else if (vitalityScore >= 70) {
      insights.push("Strong overall health with room for targeted improvements.");
    } else if (vitalityScore >= 50) {
      insights.push("Solid foundation with opportunities to optimize key areas.");
    } else {
      insights.push("Great opportunity to make meaningful improvements to your health.");
    }

    // Heart Health Insights
    if (metrics.restingHeartRate < 60 && metrics.heartRateVariability > 40) {
      insights.push("Your cardiovascular fitness is excellent - keep up the great work!");
    } else if (metrics.restingHeartRate > 80) {
      insights.push("Your resting heart rate suggests room for cardiovascular improvement.");
    }

    // Sleep Insights
    const totalSleep = metrics.deepSleepPercentage + metrics.remSleepPercentage;
    if (totalSleep > 35 && metrics.sleepConsistency > 80) {
      insights.push("Your sleep quality is excellent with good deep and REM sleep.");
    } else if (metrics.deepSleepPercentage < 15) {
      insights.push("Consider optimizing your sleep environment for better deep sleep.");
    }
    ...
    if (trends.heartHealthTrend === 'improving') {
      insights.push("Great news! Your cardiovascular health is trending upward.");
    }
    ...
    if (trends.activityTrend === 'improving') {
      insights.push("Your increased activity levels are making a positive impact.");
    }
  ...
  private generateRecommendations(
    ...
    if (metrics.restingHeartRate > 75) {
      recommendations.push("Try adding 20-30 minutes of moderate cardio 3-4 times per week.");
    }
    if (metrics.heartRateVariability < 30) {
      recommendations.push("Consider stress-reduction techniques like meditation or yoga.");
    }

    // Sleep Recommendations
    if (metrics.deepSleepPercentage < 15) {
      recommendations.push("Optimize your sleep environment: cool, dark, and quiet.");
      recommendations.push("Avoid screens 1-2 hours before bedtime.");
    }
    if (metrics.sleepConsistency < 70) {
      recommendations.push("Try maintaining a consistent sleep schedule, even on weekends.");
    }
    ...
    if (trends.heartHealthTrend === 'declining') {
      recommendations.push("Focus on cardiovascular exercise to reverse the declining heart health trend.");
    }
    if (trends.sleepTrend === 'declining') {
      recommendations.push("Review your sleep hygiene and consider what might be affecting your sleep quality.");
    }
    if (trends.activityTrend === 'declining') {
      recommendations.push("Set achievable activity goals to get back on track with your fitness routine.");
    }
  ...
  private identifyTopConcerns(
    ...
    if (metrics.heartRateVariability < 25) concerns.push("Low heart rate variability");
    ...
    if (trends.activityTrend === 'declining') concerns.push("Decreasing activity levels");
  ...
  private identifyStrengths(
    ...
    if (metrics.restingHeartRate < 65) strengths.push("Excellent resting heart rate");
    if (metrics.heartRateVariability > 40) strengths.push("Good heart rate variability");
    if (metrics.vo2Max > 45) strengths.push("High cardiovascular fitness");
    if (metrics.deepSleepPercentage > 20) strengths.push("Good deep sleep quality");
    ...
    if (trends.activityTrend === 'improving') strengths.push("Increasing activity levels");
